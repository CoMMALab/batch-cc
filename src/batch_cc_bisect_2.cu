...
// basic 1 point check prune kernel
//   (midpoint test, 32 envs per block)
//   (compacts surviving pairs into work_pairs array)
...
)
{
    ...
    float start[dim], delta[dim], point[dim];

    #pragma unroll
    for (int d = 0; d < dim; ++d) {
        start[d] = edges[0][d][edge_idx];
        delta[d] = edges[1][d][edge_idx] - start[d];
        point[d] = start[d] + delta[d] * pct_along_edge;
    }

    /* midpoint collision test */
    bool coll = not ppln::collision::fkcc<Robot>(point, envs[env_idx], /*lane=*/0);
              ...
              std::vector<bool>& results)
{
    ...
    thrust::transform(
        thrust::device,
        first, first + total_pairs,
        d_pairs_in.begin(),
        [num_envs] __device__ (int flat) {
            return WorkPair{ flat / num_envs , flat % num_envs };
        });

    /* (b) in-place copy_if to keep flag==0 (needs full check) */
    auto new_end = thrust::copy_if(
        thrust::device,
        d_pairs_in.begin(), d_pairs_in.end(),
        // d_flag,                        /* stencil */
        d_cc_full,                  /* stencil */
        d_pairs_in.begin(),            /* destination in-place */
        [] __device__ (bool f){ return f == false; });

    int num_remaining = static_cast<int>(new_end - d_pairs_in.begin());
    ...
    num_remaining = static_cast<int>(new_end - d_pairs_in.begin());
    ...
    if (num_remaining > 0) {
        ...
        fine_kernel<Robot><<<blocks2, threads2>>>(
            d_envs_ptr, d_edges_ptr,
            thrust::raw_pointer_cast(d_pairs_in.data()),
            num_remaining,
            d_cc_full,
            num_envs,
            resolution);
