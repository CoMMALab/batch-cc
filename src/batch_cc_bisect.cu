...
__global__ void prune_midpoint_kernel(
        ...
        Flag* flag /* output */)
{
    ...
    float q0[dim], q1[dim], q_mid[dim];

    #pragma unroll
    for (int d = 0; d < dim; ++d) {
        q0[d] = edges[0][d][edge_idx];
        q1[d] = edges[1][d][edge_idx];
        q_mid[d] = 0.5f * (q0[d] + q1[d]);
    }

    /* midpoint collision test */
    bool coll = not ppln::collision::fkcc<Robot>(q_mid, envs[env_idx], /*lane=*/0);
